...
public class DList {
    ...
    /**
     * Ctor1 Creates an empty DList. For an empty DList, mHead = null, mTail = null, and mSize = 0.
     */
    public DList() {
        setHead(null);
        setTail(null);
    ...
    /**
     * Ctor2 Creates a new DList with one element containing pData.
     */
    ...
    /**
     * removes all occurences of a specific integer from the list
     * @param pData
     */
    public void removeAll(Integer pData){
        int i = 0;
        while(i< getSize()){
            if(getNodeAt(i).getData() == pData){
                remove(i);
            }else{
                i++;
            }
    ...
    */
    protected Integer removeHead() {
        ...
        // Are we removing the head from a list of size 1?
        // Yes, so set the head and tail references to be null.
    ...
    */
    protected Integer removeTail() {
        ...
        if (getSize() == 1) {
            setHead(null);
            setTail(null);
        // Are we removing the tail from a list of size 1?
        // Yes, so set the head and tail references to be null.
        // This creates an empty list.
         } else {
